...
void CMBrace::Layout(CDC *pDC)
{
	...
	if (m_BraceReplace1>0)
		sz2=pDC->GetTextExtent(CString((char)m_BraceReplace1,1));
	else
		sz2=pDC->GetTextExtent(CString((char)m_BraceReplace2,1));
	pDC->SelectObject(pOldFont);
	newFont.DeleteObject();
...
void CMBrace::Draw(CDC *pDC)
{
	...
	if (m_bUseReplace){
		...
		if (m_nType<=7)
			pDC->TextOut(x,y, (char)m_BraceReplace1);
		if (m_nType<4)
			pDC->TextOut(x+m_BraceWidth+GetBox(0)->GetWidth(),y, CString((char)m_BraceReplace2, 1));
		else
			if (m_nType>7)
				pDC->TextOut(x+GetBox(0)->GetWidth(),y,CString((char)m_BraceReplace2,1));
		pDC->SelectObject(pOldFont);
		newFont.DeleteObject();
